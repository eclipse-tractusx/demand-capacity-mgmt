...
package org.eclipse.tractusx.demandcapacitymgm.backend.utils;
...
import org.eclipse.tractusx.demandcapacitymgm.backend.entities.bottlenecks.CategoryDeltaDto;
import org.eclipse.tractusx.demandcapacitymgm.backend.entities.bottlenecks.MonthReportDto;
import org.eclipse.tractusx.demandcapacitymgm.backend.entities.bottlenecks.WeekReportDto;
import org.eclipse.tractusx.demandcapacitymgm.backend.entities.bottlenecks.YearReportDto;
...
public class BottleneckDetectorUtil implements BottleneckManager {
    ...
    private List<MaterialDemandEntity> fetchDemandsBasedOnRole(UserEntity user, String userID) {
        ...
        if (user.getRole().equals(Role.CUSTOMER)) {
            demands = materialDemandRepository
                ...
                .filter(
                    d ->
                        d
                            ...
                            .allMatch(
                                series ->
                                    series.getDemandSeriesValues().stream().allMatch(value -> value.getDemand() == 0)
                            )
                )
    ...
    public YearReportResponse generateYearReport(
        ...
        for (int year = startDate.getYear(); year <= endDate.getYear(); year++) {
            ...
            List<LinkedCapacityGroupMaterialDemandEntity> matchedEntities =
                matchedDemandsRepository.findByCapacityGroupID(cgs.getId());
